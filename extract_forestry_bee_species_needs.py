#!/usr/bin/env python3
"""
Extract humidity and temperature needs for each bee species from Forestry BeeDefinition.java
"""
...
def extract_bee_species_needs(java_file_path: str) -> list[dict]:
    """
    Parse the BeeDefinition.java file and extract humidity/temperature for each species.
    ...
    enum_section_match = re.search(r'public enum BeeDefinition.*?\{(.*?)private static final EnumSet',
                                    content, re.DOTALL)
    ...
    bee_pattern = re.compile(
        r'^\s*([A-Z][A-Z0-9_]*)\s*\(\s*BeeBranchDefinition\.(\w+)',
        re.MULTILINE
    )
...
def main():
    java_file = Path(__file__).parent / "ForestryMC/src/main/java/forestry/apiculture/genetics/BeeDefinition.java"
    ...
    output_txt = Path(__file__).parent / "forestry_bee_species_needs.txt"
